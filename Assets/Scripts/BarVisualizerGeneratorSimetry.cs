...
public class BarVisualizerGeneratorSimetry : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < nBars / 2; i++)
        {
            createBar(j, i);
            j++;
        }
        for (int i = nBars / 2; i > 0; i--)
        {
            createBar(j, i);
            j++;
        }
    ...
    private void createBar(int position, int index)
    {
        ...
        asc.bias = (nBars-2*(index-1))*audioRangeDelta;
